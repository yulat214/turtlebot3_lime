...
def generate_launch_description():
    ...
    _default_gz_cm_yaml = os.path.join(
        get_package_share_directory('turtlebot3_lime_bringup'),
        'config', 'gazebo_controller_manager.yaml'
    )
    declared_arguments.append(
        DeclareLaunchArgument(
            'gz_cm_yaml_path',
            default_value=_default_gz_cm_yaml,
            description='Path to the controller_manager YAML with robot_description embedded '
                        '(generated by gazebo.launch.py for gz_ros2_control).'
        )
    )
    ...
    gz_cm_yaml_path = LaunchConfiguration('gz_cm_yaml_path')
    ...
    # In sim mode, spawners are launched from gazebo.launch.py after gz_spawn
    # completes, giving gz_ros2_control time to finish hardware initialization.
